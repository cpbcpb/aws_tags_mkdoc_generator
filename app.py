        ...
        for tag in instance["Tags"]:
            ...
            if theKey in tag_dict:
                if theValue in tag_dict[theKey]:
                    ...
                    tag_dict[theKey][theValue]["Instances"].append(instance['InstanceId'])
                else:
                    tag_dict[theKey][theValue] = {"Instances": ["%s" % instance['InstanceId']]}
            else:
                # Add new tag key to the tag dict.
                tag_dict[ theKey ]={ theValue: {"Instances": ["%s" % instance['InstanceId']]}}
                # else:
                #     
print(tag_dict)  # Todo: obviously this doesn't quite work, need to go key by key.
# a_file=open("./output/all_tags")
# a_file.write("##%s\n" & tag[Key]")
